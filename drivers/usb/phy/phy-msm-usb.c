		...
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_SDP_CHARGER:
					pr_info("xxxxxxxxxxxUSB_SDP_CHARGERxxxxxxxxxxxxx");//add by zhangjiyou for nonstandard charger, @20151210
					msm_otg_start_peripheral(otg, 1);
					otg->phy->state =
						OTG_STATE_B_PERIPHERAL;
					mod_timer(&motg->chg_check_timer,
							CHG_RECHECK_DELAY);
					msm_otg_notify_charger(motg, IDEV_ACA_CHG_LIMIT);//add by zhangjiyou for nonstandard charger, @20151210
					break;
				default:
					break;
				}
