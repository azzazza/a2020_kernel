...
int msg_firmware_upgrade(struct firmware_miscdevice *mdev, const char __user *buff, int size) 
{
	int ret = 0;
...
#endif //CONFIG_ENABLE_HOTKNOT
/*******add for factory check fw like as Q05*****************************/
#ifdef CONFIG_TP_FW_ID
static ssize_t name_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    ...
    DBG("*** %s() _gFwVersion = %s ***\n", __func__, _gFwVersion);
    nLength += sprintf(buf, "%02x%02x\n", nMajor,nMinor );

    return nLength;
}

static struct kobj_attribute tp_fw = __ATTR_RO(name);

static struct attribute *fw_attrs[] = {
	&tp_fw.attr,
	NULL,
};
static struct attribute_group attr_fw_group = {
	.attrs = fw_attrs,
};

static void Tp_fw_show(void)
{
        int retval;
        struct kobject *currentfw;

        currentfw = kobject_create_and_add("tp_info", NULL);
        if (!currentfw)
		printk(KERN_WARNING "[%s] failed to create a sysfs kobject\n",__func__);
	else{	
        	retval = sysfs_create_group(currentfw,
                                &attr_fw_group);
        	if (retval){
              	 	kobject_put(currentfw);
			printk(KERN_WARNING "[%s] failed to create a sysfs group %d\n",__func__, retval);
       		 }
	}
}
#endif
/*******add for factory check fw*****************************/
...
s32 DrvMainTouchDeviceInitialize(void)
{
    ...
    {
        SLAVE_I2C_ID_DBBUS = (0xC4>>1); //0x59
