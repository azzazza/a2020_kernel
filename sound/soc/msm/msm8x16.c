			...
			struct msm8916_asoc_mach_data *pdata)
{
	...
	if (pdata->us_euro_gpio < 0) {
		...
		if (rc) {
			pr_err("%s: Failed to request tapan us-euro gpio %d\n",
...
static ssize_t hstype_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
	struct snd_soc_card *card = dev_get_drvdata(dev);
	struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);

	pr_err("%s enter,pdata->headset_type=%d\n", __func__,pdata->headset_type);
	return sprintf(buf, "%d\n", pdata->headset_type);
}

static DEVICE_ATTR(hstype, 0644, hstype_show, NULL);

static const struct attribute *msm8x16_asoc_attr[] = {
     &dev_attr_hstype.attr,
     NULL,
};

static const struct attribute_group msm8x16_asoc_attr_group = {
	.attrs = (struct attribute **)msm8x16_asoc_attr,
};
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	/* Add by XiangSheng for bug87416, 20151105 start */
	ret = sysfs_create_group(&pdev->dev.kobj, &msm8x16_asoc_attr_group);
	if (ret) {
