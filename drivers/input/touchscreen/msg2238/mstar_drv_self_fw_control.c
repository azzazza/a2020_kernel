...
static u8 preTouchStatus = 0; //Previous Touch VA Status;
        ...
        if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF) && (pPacket[6] == 0xFF))
        {
            ...
            {   /* 0x00 is key up, 0xff is touch screen up */
                ...
                if (g_EnableTpProximity && ((pPacket[5] == 0x80) || (pPacket[5] == 0x40)))
                {
                    ...
                    {
                        g_FaceClosingTp = 0;
						input_report_abs(ps_inputdevice, ABS_DISTANCE, g_FaceClosingTp);
						input_sync(ps_inputdevice);		
                    }
...
//    DBG("*** %s() ***\n", __func__);  // add for debug
    ...
    if (g_FbNotifyCallback == 1)
    {
        DBG("Skip read touch packet handling when TP Resume/Suspend is triggerd.\n");
        goto TouchHandleEnd;		
    }
