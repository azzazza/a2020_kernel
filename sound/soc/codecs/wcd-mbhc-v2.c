...
struct wcd_mbhc *m_mbhc;
static int plug_special_headset = 0;
static bool need_swap=false; //add by yuzhaohua for detecting headset on 20150105
					 ...
					 struct work_struct *work)
{
	...
	if (plug_special_headset==1)
	{
		plug_special_headset=0;
	}
	...
	* removal event to sync-up PA's state */
	if (wcd_mbhc_is_hph_pa_on(codec)) {
		...
		set_bit(WCD_MBHC_HPHR_PA_OFF_ACK, &mbhc->hph_pa_dac_state);
			snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_RX_HPH_CNP_EN,//changed by yuzhaohua on 20151119 
			    0x30, 0x10);
	} else {
		pr_debug("%s PA is off\n", __func__);
	snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_RX_HPH_CNP_EN,
			    0x30, 0x00);//changed by yuzhaohua on 20151119
	}
	usleep_range(wg_time * 1000, wg_time * 1000 + 50);
}
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (jack_type == SND_JACK_HEADPHONE)
			...
			mbhc->current_plug = MBHC_PLUG_TYPE_HIGH_HPH;
#if 0  //deleted by yuzhaohua on 20151231 for it is only used for debug
					 ...
					 enum wcd_mbhc_plug_type plug_type)
{
	...
	if ((plug_special_headset==1)&&(plug_type==MBHC_PLUG_TYPE_GND_MIC_SWAP)) // changed by yuzhaohua on 20151231
	{
		plug_special_headset=0;
	}
	WCD_MBHC_RSC_ASSERT_LOCKED(mbhc);
// changed by yuzhaohua on 20151231 start 
	if ((mbhc->current_plug == plug_type)&&((plug_type==MBHC_PLUG_TYPE_HEADSET)||
	(plug_type==MBHC_PLUG_TYPE_HEADPHONE)||(plug_type==MBHC_PLUG_TYPE_HIGH_HPH)) ){
		pr_debug("%s: cable already reported, exit\n", __func__);
		goto exit;
	}
// changed by yuzhaohua on 20151231 end
	if (plug_type == MBHC_PLUG_TYPE_HEADPHONE) {
		...
		if (mbhc->impedance_detect) {
			...
			if ((mbhc->zl > 20000) && (mbhc->zr > 20000)) {
				pr_err("%s: special accessory 1\n", __func__);
				/* Toggle switch back */
				 if(need_swap)  //add by yuzhaohua for detecting headset on 20150105
				if (mbhc->mbhc_cfg->swap_gnd_mic &&
					mbhc->mbhc_cfg->swap_gnd_mic(codec)) {
					pr_debug("%s: US_EU gpio present,flip switch again\n"
							, __func__);
				}
				if (plug_special_headset==0)
				{
					wcd_enable_curr_micbias(mbhc,WCD_MBHC_EN_MB);//WCD_MBHC_EN_MB
					plug_special_headset=1;
					pr_err("%s: WCD_MBHC_EN_MB 1\n", __func__);
...
// added by yuzhaohua for bug100948 on 20151231 start 
       reg1 = snd_soc_read(codec, MSM8X16_WCD_A_ANALOG_RX_HPH_CNP_EN);
	if(reg1 & 0x30) plug_type = MBHC_PLUG_TYPE_GND_MIC_SWAP;
// added by yuzhaohua for bug100948 on 20151231 end   
...
/* Add by XiangSheng for bug87416, 20151105 start */
static void ftm_store_headset_state(struct wcd_mbhc *mbhc)
{
    int value = 0; 
    struct snd_soc_card *card = mbhc->codec->card;
    struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);

    value = gpio_get_value_cansleep(pdata->us_euro_gpio);
    pr_debug("%s: value = %d gpio = %d\n", __func__, value, pdata->us_euro_gpio);
	//add by yuzhaohua for  headset test on 20150105 start
    if(value)
    pdata->headset_type = HEADSET_TYPE_OMTP;  
    else
    pdata->headset_type =HEADSET_TYPE_CTIA;
	//add by yuzhaohua for  headset test on 20150105 end
}
/* Add by XiangSheng for bug87416, 20151105 end */
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
    ...
    need_swap=true; 	//add by yuzhaohua for  detecting headset on 20150105 
			...
			if (!(plug_type == MBHC_PLUG_TYPE_GND_MIC_SWAP)) {
				...
				if (mbhc->current_plug !=
					...
					pr_debug("%s: cable is headset\n",
							__func__);
					need_swap=false;  	//add by yuzhaohua for  detecting headset on 20150105 
					goto report;
				}
			}
			wrk_complete = false;
	...
	if (!wrk_complete && plug_type == MBHC_PLUG_TYPE_HEADSET) {
		...
		goto enable_supply;  //change to origin  by yuzhaohua on 20151231
	}
...
{
	struct snd_soc_codec *codec = mbhc->codec;
	long timeout = msecs_to_jiffies(50);  //50ms /* 50ms */
	enum wcd_mbhc_plug_type plug_type= MBHC_PLUG_TYPE_INVALID; //changed  by yuzhaohua on 20151231
	int timeout_result;
	u16 result1, result2;
	bool micbias1;
	//deleted  by yuzhaohua on 20151231	start
//	bool cross_conn;  
//	int try = 0;
	//deleted  by yuzhaohua on 20151231	end
	pr_debug("%s: enter\n", __func__);
	WCD_MBHC_RSC_ASSERT_LOCKED(mbhc);
	micbias1 = (snd_soc_read(codec, MSM8X16_WCD_A_ANALOG_MICB_1_EN) & 0x80);
	if (mbhc->mbhc_cb && mbhc->mbhc_cb->set_cap_mode)
	...
	if (!timeout_result) {
		...
		if (!(result2 & 0x01)) {
			 ...
			 */
			 #if 0 //changed  by yuzhaohua on 20151231 start
			do {
...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	struct snd_soc_card *card = mbhc->codec->card;
       struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	pr_debug("%s: enter\n", __func__);
       pdata->headset_type =HEADSET_TYPE_UNKNOWN;
	   	//add by yuzhaohua for  headset test on 20150105 end
	WCD_MBHC_RSC_LOCK(mbhc);
	...
	if(detection_type){
		if(mbhc->regVdd != NULL){
			if (regulator_enable(mbhc->regVdd)) {
				printk("qiaodeli *** %s() *** Regulator g_ReguVdd enable failed \n",__func__);
			}
		}
	}else{
		if(mbhc->regVdd != NULL){
			if (regulator_disable(mbhc->regVdd)) {
				printk("qiaodeli *** %s() *** Regulator g_ReguVdd disable failed \n",__func__);
			}
		}
	}
	msleep(20);
...
static int wcd_mbhc_get_button_mask(u16 btn)
{
	int mask = 0;
	pr_debug("%s: enter %d\n", __func__, btn);
	if((btn==1)&&(plug_special_headset==1)){
		btn=3;
		} 
	//added by yuzhaohua for bug 101032 & 100972 on 20151231 start
	if((btn==1)&&(plug_special_headset==0)){
		btn=2; 
		}
   	//added by yuzhaohua for bug 101032 & 100972 on 20151231 end		
	switch (btn) {
	case 0:
		mask = SND_JACK_BTN_0;
		break;
...
static void mbhc_suspend(void)
{
	printk("%s\n", __func__);


	if(m_mbhc->regVdd != NULL){
		printk("mbhs regulator_disable\n");
//		if (regulator_disable(m_mbhc->regVdd)) {
			printk("qiaodeli *** %s() *** Regulator g_ReguVdd disable failed \n",__func__);
//		}
	}
}

static void mbhc_resume(void)
{
	printk("%s\n", __func__);

	if(m_mbhc->regVdd != NULL){
		printk("mbhs regulator_enable\n");
//		if (regulator_enable(m_mbhc->regVdd)) {
			printk("qiaodeli *** %s() *** Regulator g_ReguVdd enable failed \n",__func__);
//		}
	}
}
/*
static struct syscore_ops mbhc_syscore_ops = {
	.suspend = mbhs_suspend,
	.resume = mbhs_resume,
};*/
		      ...
		      bool impedance_det_en)
{
	...
	mbhc->skip_imped_detection = false;
	mbhc->regVdd = NULL;
	mbhc->suspend = mbhc_suspend;
	mbhc->resume = mbhc_resume;
	...
	mbhc->regVdd = regulator_get(card->dev, vdd_name);
	if(mbhc->regVdd != NULL) {
/*		if (regulator_enable(mbhc->regVdd)) {
				printk("qiaodeli *** %s() *** Regulator g_ReguVdd enable failed \n",__func__);
		}
    		regulator_set_voltage(mbhc->regVdd, 2850000, 2850000); */
	}
//	register_syscore_ops(&mbhc_syscore_ops);
