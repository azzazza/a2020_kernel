...
struct qpnp_lbc_chip {
	...
	bool				set_max_current_flag; //zhangjiyou add for limit max current when it is calling, @20151103
	bool				usbin_limit_current_flag; //zhangjiyou add for low the current when the battery enconter high charger voltage, @20151118
	...
	struct delayed_work		capacity_full_work;//zhangjiyou add for bug81252, @20151009
};
...
static int qpnp_lbc_ibatmax_set(struct qpnp_lbc_chip *chip, int chg_current)
{
	...
	/*zhangjiyou add for low the current when the battery enconter high charger voltage, @20151118, start*/
	if (chip->usbin_limit_current_flag){
		pr_info("Because of usbin ov, have limited the current, Now current = %d\n", chip->usb_psy_ma);
		return 0;
	}
	/*zhangjiyou add for low the current when the battery enconter high charger voltage, @20151118, end*/

	/*zhangjiyou add for limit max current when it is calling, @20151103, start*/
	if (chip->set_max_current_flag){
		pr_info("have set the max current from User space Now current = %d\n", chip->usb_psy_ma);
		return 0;
	}
	/*zhangjiyou add for limit max current when it is calling, @20151103, end*/
...
/*zhangjiyou add for low the current when the battery enconter high charger voltage, @20151118, start*/
static int get_prop_usbin_voltage_now(struct qpnp_lbc_chip *chip)
{
	int rc = 0;
	struct qpnp_vadc_result results;
...
static int get_prop_batt_temp(struct qpnp_lbc_chip *chip)
{
	...
	if(POWER_SUPPLY_STATUS_DISCHARGING == get_prop_batt_status(chip)) {
		//if ((int)results.physical >= -50 && (int)results.physical <= 50)
		//	increase = -10;
		//else if ((int)results.physical >= 410 && (int)results.physical <= 500)
		if ((int)results.physical >= 410 && (int)results.physical <= 500)
			increase = (400 - (int)results.physical) / 10;
		else if ((int)results.physical > 500 && (int)results.physical <= 550)
			increase = -10;
		else 
			increase = 0;
	}
	return (increase + (int)results.physical);
					...
					enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_MAX_CURRENT:
		return 1;
	default:
		break;
	}
 ...
 * b. disable charging
 */
#define TIME_OF_FROM_100_TO_FULL (4 * 60 * 1000) //zhangjiyou add for bug81252, @20151009
				...
				const union power_supply_propval *val)
{
	struct qpnp_lbc_chip *chip = container_of(psy, struct qpnp_lbc_chip,
								batt_psy);
	int rc = 0;
	union power_supply_propval ret = {0,}; //zhangjiyou add for limit max current when it is calling, @20151103
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		case POWER_SUPPLY_STATUS_FULL:
			if (chip->cfg_float_charge)
				break;
			/* Disable charging */
			/*zhangjiyou add for bug81252, @20151009, start*/
			cancel_delayed_work(&chip->capacity_full_work);
			pr_info("After %d minutes ,charger will stop charging\n", TIME_OF_FROM_100_TO_FULL / 60000);
			schedule_delayed_work(&chip->capacity_full_work, msecs_to_jiffies(TIME_OF_FROM_100_TO_FULL));
			/*zhangjiyou add for bug81252, @20151009, end*/
	...
	case POWER_SUPPLY_PROP_MAX_CURRENT:
		...
		if (!qpnp_lbc_is_usb_chg_plugged_in(chip) || POWER_SUPPLY_TYPE_USB == ret.intval)
			break;
		chip->usb_psy_ma = val->intval;
		if (val->intval > 500)
			chip->set_max_current_flag = 0;
		pr_info("setting charger current %d mA\n", val->intval);
		rc = qpnp_lbc_ibatmax_set(chip, val->intval);
		if (!rc && val->intval <= 500)
			chip->set_max_current_flag = 1;
		break;
	/*zhangjiyou add for limit max current when it is calling, @20151103, end*/
	default:
		return -EINVAL;
...
static irqreturn_t qpnp_lbc_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	chip->set_max_current_flag = 0; //zhangjiyou add for limit max current when it is calling, @20151103
	chip->usbin_limit_current_flag = 0; //zhangjiyou add for low the current when the battery enconter high charger voltage, @20151118

	if (chip->usb_present ^ usb_present) {
		chip->usb_present = usb_present;
		if (!usb_present) {
			cancel_delayed_work(&chip->capacity_full_work);//zhangjiyou add for bug81252, @20151009
			qpnp_lbc_charger_enable(chip, CURRENT, 0);
			spin_lock_irqsave(&chip->ibat_change_lock, flags);
			chip->usb_psy_ma = QPNP_CHG_I_MAX_MIN_90;
			qpnp_lbc_set_appropriate_current(chip);
			...
			qpnp_lbc_charger_enable(chip, SOC, 1);
			/*zhangjiyou add for bug81252, @20151009, start*/
			pr_info("After %d minutes ,charger will check again\n", TIME_OF_FROM_100_TO_FULL / 60000);
			schedule_delayed_work(&chip->capacity_full_work, msecs_to_jiffies(TIME_OF_FROM_100_TO_FULL));
			/*zhangjiyou add for bug81252, @20151009, end*/
		}
...
/*zhangjiyou add for low the current when the battery enconter high charger voltage, @20151118, start*/
#define USBIN_OV_CURRENT	630
#define USBIN_NORMAL_CURRENT	810
/*zhangjiyou add for low the current when the battery enconter high charger voltage, @20151118, end*/
static void qpnp_lbc_vddtrim_work_fn(struct work_struct *work)
{
	...
	if (!qpnp_lbc_is_fastchg_on(chip) ||
		...
		/*zhangjiyou add for low the current when the battery enconter high charger voltage, @20151118, start*/
			...
			/*First time to set*/
			if (!chip->usbin_limit_current_flag && get_prop_usbin_voltage_now(chip) > 6000000) {
				qpnp_lbc_ibatmax_set(chip, USBIN_OV_CURRENT);
				chip->usbin_limit_current_flag = 1;
				pr_info("First time to set\n");
			/*Exception to keep*/
			} else if (chip->usbin_limit_current_flag && get_prop_usbin_voltage_now(chip) > 6000000){
				pr_info("Exception to keep\n");
			/*Become normal to reset*/
			} else if (chip->usbin_limit_current_flag && get_prop_usbin_voltage_now(chip) < 5500000){
				chip->usbin_limit_current_flag = 0;			
				qpnp_lbc_ibatmax_set(chip, USBIN_NORMAL_CURRENT);
				pr_info("Become normal to reset\n");
			/*Normal continuing*/
			} else {
				//pr_info("Normal continuing\n");
			}
		}
		/*zhangjiyou add for low the current when the battery enconter high charger voltage, @20151118, end*/
...
/*zhangjiyou add for bug81252, @20151009, start*/
static int battery_full_work_flag = 0;
static void qpnp_lbc_capacity_full_work_fn(struct work_struct *work)
{
	int rc;
	int current_now;
	int voltage_now;
	struct qpnp_lbc_chip *chip = container_of(work, struct qpnp_lbc_chip,
						capacity_full_work.work);

	current_now = get_prop_current_now(chip);
	voltage_now = get_prop_battery_voltage_now(chip);
	pr_info("chip->chg_done = %d status = %d soc=%d current = %d voltage = %d\n", 
		chip->chg_done, get_prop_batt_status(chip), get_prop_capacity(chip), current_now, voltage_now);

	if (get_prop_capacity(chip) < 100) {
		battery_full_work_flag = 0;
		return ;
	}
	if(chip->chg_done == true) {
		battery_full_work_flag = 0;
		return ;
	}
	if(get_prop_batt_status(chip) == POWER_SUPPLY_STATUS_DISCHARGING) {
		battery_full_work_flag = 0;
		return ;
	}
	/*zhangjiyou add for battery full, @20151124, start*/
	if (current_now > -60000 || voltage_now > 4350000) {
		if (++battery_full_work_flag > 1) {
			battery_full_work_flag = 0;
			if (voltage_now > 4343000) {
				rc = qpnp_lbc_charger_enable(chip, SOC, 0);
				if (rc)
					pr_err("Failed to disable charging rc=%d\n",
							rc);
				else {
					pr_info("Current less 60ma twice & Voltage more 4345 and Stop charging!\n");
					chip->chg_done = true;
					power_supply_changed(&chip->batt_psy);
				}
			} else {
			...
			pr_info("battery_full_work_flag = %d\n", battery_full_work_flag);
			pr_info("After %d minutes ,charger will check again\n", TIME_OF_FROM_100_TO_FULL / 60000);
			schedule_delayed_work(&chip->capacity_full_work, msecs_to_jiffies(TIME_OF_FROM_100_TO_FULL));
		}
	} else {
	...
	/*zhangjiyou add for battery full, @20151124, end*/
}
/*zhangjiyou add for bug81252, @20151009, end*/
...
static int qpnp_lbc_main_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->capacity_full_work, qpnp_lbc_capacity_full_work_fn);//zhangjiyou add for bug81252, @20151009
...
static int qpnp_lbc_remove(struct spmi_device *spmi)
{
	...
	cancel_delayed_work_sync(&chip->capacity_full_work);//zhangjiyou add for bug81252, @20151009
