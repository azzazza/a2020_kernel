...
static void set_gpio_enable(struct msm8x16_wcd_priv *msm8x16_wcd)
{
	...
	printk(KERN_ERR "%s:in ANGUS3_DOMESTIC\n", __func__);
    ...
    printk(KERN_ERR "%s:in ANGUS3_OVERSEAS\n", __func__);
	gpio_direction_output(ext_spk_pa_gpio, 1);  
    udelay(2);
    gpio_direction_output(ext_spk_pa_gpio, 0);  
    udelay(2);
...
int msm8x16_wcd_suspend(struct snd_soc_codec *codec)
{
	...
	struct msm8x16_wcd_priv *msm8x16_wcd_priv = (struct msm8x16_wcd_priv *)snd_soc_codec_get_drvdata(codec);
	
	pdata = snd_soc_card_get_drvdata(codec->card);
	printk("msm8x16_wcd_suspend\n");
	if(msm8x16_wcd_priv->mbhc.suspend) 
		msm8x16_wcd_priv->mbhc.suspend();
	else
		printk("msm8x16_wcd_priv->mbhc no suspend\n");
...
int msm8x16_wcd_resume(struct snd_soc_codec *codec)
{
	...
	struct msm8x16_wcd_pdata *msm8x16_pdata = msm8x16->dev->platform_data;
	struct msm8x16_wcd_priv *msm8x16_wcd_priv = (struct msm8x16_wcd_priv *)snd_soc_codec_get_drvdata(codec);
	printk("msm8x16_wcd_resume\n");
	if(msm8x16_wcd_priv->mbhc.resume) 
		msm8x16_wcd_priv->mbhc.resume();
	else
		printk("msm8x16_wcd_priv->mbhc no resume\n");
	pdata = snd_soc_card_get_drvdata(codec->card);
