...
struct qpnp_bms_chip {
	...
	/* zhangjiyou add for temp update to Android, @20150922, start*/
	int				last_temp;
	int64_t			temp_jiffies;
	/* zhangjiyou add for temp update to Android, @20150922, end*/
	/*zhangjiyou add for fake soc when warm charge @20151020, start*/
	int				fake_capacity;
	int64_t			fake_jiffies;
	/*zhangjiyou add for fake soc when warm charge @20151020, end*/
	int64_t			report_soc_jiffies; //zhangjiyou add for soc changing fast on lenovo jar bug, @20151113
};
...
static int get_batt_therm(struct qpnp_bms_chip *chip, int *batt_temp)
{
	int rc;
	struct qpnp_vadc_result result;
	int increase = 0; //zhangjiyou add for correcting the temp @20151014
	...
	/*zhangjiyou add for correcting the temp @20151014, start*/
	if(POWER_SUPPLY_STATUS_DISCHARGING == get_battery_status(chip)) {
		//if ((int)result.physical >= -50 && (int)result.physical <= 50)
		//	increase = -10;
		//else if ((int)result.physical >= 410 && (int)result.physical <= 500)
		if ((int)result.physical >= 410 && (int)result.physical <= 500)
			increase = (400 - (int)result.physical) / 10;
		else if ((int)result.physical > 500 && (int)result.physical <= 550)
			increase = -10;
		else 
			increase = 0;
	}
	*batt_temp = increase + (int)result.physical;
	/*zhangjiyou add for correcting the temp @20151014, end*/

	/* zhangjiyou add for temp update to Android, @20150922, start*/
	pr_debug("last_temp = %d ,batt_temp phy = %lld meas = 0x%llx\n",
			chip->last_temp, result.physical, result.measurement);
	
	if (chip->last_temp != (int)result.physical 
		&& (get_jiffies_64() - chip->temp_jiffies > 20 * HZ)) {
		chip->last_temp = (int)result.physical;
		chip->temp_jiffies = get_jiffies_64();
...
static int charge_cnt = 0; //zhangjiyou add for bug103000, @20160113
static void charging_began(struct qpnp_bms_chip *chip)
{
	...
	/*zhangjiyou add for bug103000, @20160113, start*/
	if (chip->last_soc != 100) {
		charge_cnt = 0;
		chip->eoc_reported = false;
		pr_info("eoc_reported = false charge_cnt = 0\n");
	}
	/*zhangjiyou add for bug103000, @20160113, end*/
...
static void check_recharge_condition(struct qpnp_bms_chip *chip)
{
	...
	/*zhangjiyou add for fake soc when warm charge @20151023, start*/
	int health = get_battery_health(chip);
	int recharge_flag = 0;
	/*zhangjiyou add for fake soc when warm charge @20151023, end*/
	...
	/*zhangjiyou add for fake soc when warm charge @20151023, start*/
	if (chip->fake_capacity >= 0){
		if ((chip->last_soc < 78) && (health == POWER_SUPPLY_HEALTH_WARM)){
			recharge_flag = 1;
		} else if (health == POWER_SUPPLY_HEALTH_GOOD || health == POWER_SUPPLY_HEALTH_COOL){
			recharge_flag = 1;
		} else {
			recharge_flag = 0;
		}
	...
	if ((status != POWER_SUPPLY_STATUS_CHARGING) && 
		(chip->eoc_reported || recharge_flag)) {
		if ((chip->last_soc >= 78) && (health == POWER_SUPPLY_HEALTH_WARM))
			return ;
	/*zhangjiyou add for fake soc when warm charge @20151020, end*/
...
static void check_eoc_condition(struct qpnp_bms_chip *chip)
{
	 ...
	 */
	if ((chip->last_soc == 100 || chip->fake_capacity == 100) && //zhangjiyou add for fake soc when warm charge @20151020
	...
	if (chip->ocv_at_100 == -EINVAL) {
		if (chip->last_soc == 100 || chip->fake_capacity == 100) { //zhangjiyou add for fake soc when warm charge @20151020
			if (chip->dt.cfg_report_charger_eoc && !chip->eoc_reported && charge_cnt >= 5) {//zhangjiyou modify for bug81252, @20151009
				rc = report_eoc(chip);
				if (!rc) {
					/*
		...
		if (chip->last_ocv_uv >= chip->ocv_at_100) {
			...
			chip->ocv_at_100 = chip->last_ocv_uv;
			if(chip->fake_capacity < 0)  //zhangjiyou add for fake soc when warm charge @20151020
				chip->last_soc = 100;
...
/*zhangjiyou add for fake soc when warm charge @20151020, start*/
#define REAL_CAPACITY 0
#define FAKE_CAPACITY 1
#define FAKE_FULL_CAPACITY 2
static int high_temp_calculate_capacity(struct qpnp_bms_chip *chip)
{
	int current_status;
	int current_health;
	
	current_status	= get_battery_status(chip);
	current_health	= get_battery_health(chip);

	/*
	 * Battery is gone
	 */
	if (!is_battery_present(chip)) {
		chip->fake_capacity = -1;
		return REAL_CAPACITY;
	}

	/*
	 * Real capacity have reached 100
	 */
	if (chip->last_soc == 100) {
		chip->fake_capacity = -1;
		return REAL_CAPACITY;
	...
	/*
	 * Time > 10s add or reduce
	 */
	if(chip->fake_capacity >= 0) {
		if(get_jiffies_64() - chip->fake_jiffies > 10 * HZ) {
			chip->fake_jiffies = get_jiffies_64();
		} else {
			return FAKE_CAPACITY;
		}
	/*
	 * Maybe Come into FAKE firstly
	 */
	} else {
		chip->fake_jiffies = get_jiffies_64();
	}

	/*zhangjiyou add for debug, @20151104, start*/
	pr_info("current_status = %d current_health = %d chip->fake_capacity = %d chip->last_soc = %d\n", 
		current_status, current_health, chip->fake_capacity, chip->last_soc);
	/*zhangjiyou add for debug, @20151104, end*/

	/* 
	 * Come into WARM status
	 * Charging
	 * Chip->fake_capacity must be less than chip->last_soc
	 * Chip->fake_capacity up
	 * Chip->last_soc keep
	 */
	if (POWER_SUPPLY_HEALTH_WARM == current_health
		&& current_status == POWER_SUPPLY_STATUS_CHARGING
		&& chip->last_soc >= 78) {

		if (chip->fake_capacity < chip->last_soc)
			chip->fake_capacity = chip->last_soc;
		else
			chip->fake_capacity++;

		if (chip->fake_capacity > 100)
			chip->fake_capacity = 100;
		if (chip->fake_capacity == 100)
			return FAKE_FULL_CAPACITY;
		else 
			return FAKE_CAPACITY;
	}
	...
	/* 
	 * Fake capacity
	 * fake & !is_charger_present
	 * Chip->fake_capacity must be more than chip->last_soc
	 * Chip->fake_capacity down
	 * Chip->last_soc down
	 */
	if (chip->fake_capacity >= 0 && ((!is_charger_present(chip)) || POWER_SUPPLY_HEALTH_OVERHEAT == current_health)) {
	 ...
	 * Charging
	 * Chip->fake_capacity must be more than chip->last_soc
	 * Chip->fake_capacity not change anymore
	 * Chip->last_soc up
	 */
	if (current_status == POWER_SUPPLY_STATUS_CHARGING && chip->fake_capacity >= 0) {
...
static int report_vm_bms_soc(struct qpnp_bms_chip *chip)
{
	...
	bool charging;
	int fake_battery_capacity; //zhangjiyou add for fake soc when warm charge @20151020
	 ...
	 */
	soc = bound_soc(soc);
	if ((soc != chip->last_soc) || (soc == 100) || (chip->fake_capacity == 100)) { 	//zhangjiyou add for fake soc when warm charge @20151020
		chip->last_soc = soc;
	...
	/*zhangjiyou add for fake soc when warm charge @20151020, start*/
	if ((POWER_SUPPLY_HEALTH_WARM == get_battery_health(chip)  
		&& get_battery_status(chip) == POWER_SUPPLY_STATUS_CHARGING
		&& chip->last_soc >= 78)
		|| chip->fake_capacity >= 0)
	{
		fake_battery_capacity = high_temp_calculate_capacity(chip);
		pr_debug("fake_battery_capacity = %d\n", fake_battery_capacity);
		if(REAL_CAPACITY != fake_battery_capacity)
...
static int report_state_of_charge(struct qpnp_bms_chip *chip)
{
	...
	/*zhangjiyou add for soc changing fast on lenovo jar bug, @20151113, start*/
	else {
		if (get_jiffies_64() - chip->report_soc_jiffies > 1 * HZ || chip->last_soc == -EINVAL){
			soc = report_vm_bms_soc(chip);
			chip->report_soc_jiffies = get_jiffies_64();
		} else {
			if (chip->fake_capacity >= 0)
				soc = chip->fake_capacity;
			else
				soc = chip->last_soc;
		}
	}
	/*zhangjiyou add for soc changing fast on lenovo jar bug, @20151113, end*/
...
static int clamp_soc_based_on_current(struct qpnp_bms_chip *chip, int soc)
{
	int vbat_current;

	if (!is_battery_charging(chip) && soc < 100) {
		charge_cnt = 0;
		return soc;
	}
	...
	vbat_current = get_prop_bms_current_now(chip) / 1000;
	if (vbat_current > -70 || chip->voltage_soc_uv > 4350000) {
		if (charge_cnt < 6)
			charge_cnt++;
	} else 
		charge_cnt = 0;

	if (charge_cnt < 5 && soc == 100)
		soc = 99;
	else if (soc == 100) 
		pr_info("soc == 100, charge_cnt >= 5\n");
	else 
		pr_info("charge_cnt = %d\n", charge_cnt);
...
static void monitor_soc_work(struct work_struct *work)
{
	...
	if (!is_battery_present(chip)) {
		...
		if (chip->dt.cfg_use_voltage_soc) {
			...
			/*zhangjiyou add for bug103000, @20160113, start*/
			new_soc = clamp_soc_based_on_current(chip, new_soc);
			/*zhangjiyou add for bug103000, @20160113, end*/

			/*zhangjiyou add for debug, @20151104, start*/
			if((chip->last_soc > 90) || (chip->calculated_soc != new_soc) || (chip->last_soc != chip->calculated_soc))
				pr_info("calculated_soc = %d, new_soc = %d, last_soc = %d, status = %d, current = %d, voltage = %d, health = %d\n", 
					chip->calculated_soc, new_soc, chip->last_soc, chip->battery_status, chip->current_now, chip->voltage_soc_uv, get_battery_health(chip));
			/*zhangjiyou add for debug, @20151104, end*/
...
static int qpnp_vm_bms_probe(struct spmi_device *spmi)
{
	...
	chip->warm_reset = !!rc;
	chip->report_soc_jiffies = get_jiffies_64(); //zhangjiyou add for soc changing fast on lenovo jar bug, @20151113
	/* zhangjiyou add for temp update to Android, @20150922, start*/
	chip->last_temp  = BMS_DEFAULT_TEMP;
	chip->temp_jiffies = get_jiffies_64();
	chip->fake_capacity = -1; 	//zhangjiyou add for fake soc when warm charge @20151020
	rc = get_batt_therm(chip, &chip->last_temp);
	if (rc < 0) {
		pr_err("Unable to read batt temp, using default=%d\n",
						BMS_DEFAULT_TEMP);
		chip->last_temp = BMS_DEFAULT_TEMP;
...
static int bms_resume(struct device *dev)
{
	...
	struct qpnp_bms_chip *chip = dev_get_drvdata(dev);
	int64_t temp_jiffies_64; //zhangjiyou add for suspend status can't update capacity, @20151215
	...
	/*zhangjiyou add for suspend status can't update capacity, @20151215, start*/
	/*
	 * bms_resume to change fake capacity
 	 */
	if(chip->fake_capacity >= 0 && chip->fake_capacity <= 100) {
		temp_jiffies_64 = (get_jiffies_64() - chip->fake_jiffies);
		pr_info("====temp_jiffies_64 = %lld, increase1 = %d, increase2 = %d\n",
			temp_jiffies_64, (int)temp_jiffies_64 / 100, (int)temp_jiffies_64 / (15 * HZ));
	 	if(get_battery_status(chip) == POWER_SUPPLY_STATUS_CHARGING) {
			if ((int)temp_jiffies_64 / 100 >= 3)
				temp_jiffies_64 = 3;
			else 
				temp_jiffies_64 =  ((int)temp_jiffies_64 / 100);
			pr_info("====(int)temp_jiffies_64 = %d\n", (int)temp_jiffies_64);
			if((int)temp_jiffies_64  > 0) {
				chip->fake_capacity += (int)temp_jiffies_64;
				if(chip->fake_capacity > 100)
					chip->fake_capacity = 100;
				chip->fake_jiffies = get_jiffies_64();
				power_supply_changed(&chip->bms_psy);
			}
		} else if(get_battery_status(chip) == POWER_SUPPLY_STATUS_DISCHARGING) {
			if ((int)temp_jiffies_64 / 300 >= 3)
				temp_jiffies_64 = 3;
			else 
				temp_jiffies_64 =  ((int)temp_jiffies_64 / 300);
			pr_info("====(int)temp_jiffies_64 = %d\n", (int)temp_jiffies_64);
			if((int)temp_jiffies_64 > 0) {
				chip->fake_capacity -= (int)temp_jiffies_64;
				if(chip->fake_capacity < chip->last_soc)
	...
	/*
	 * capacity == 0 update to Android to shutdown system
	 */
	else if(chip->last_soc == 0 && get_battery_status(chip) == POWER_SUPPLY_STATUS_DISCHARGING)
		power_supply_changed(&chip->bms_psy);
	/*zhangjiyou add for suspend status can't update capacity, @20151215, start*/
