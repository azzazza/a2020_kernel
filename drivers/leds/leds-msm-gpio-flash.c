...
struct led_gpio_flash_data {
	...
	struct pinctrl_state *gpio_flash_type_state_default;
...
static int flashTypevale = 0; 
#ifdef ANGUS3_OVERSEAS
				    ...
				    enum led_brightness value)
{
	...
	if (brightness > LED_HALF) {
		#ifdef ANGUS3_DOMESTIC
		gpio_direction_output(flash_led->flash_now, 1);
		#endif

		#ifdef ANGUS3_OVERSEAS
		if(flashTypevale == 1)
		led_ktd262_brightness_set(led_cdev,16);  //16-->1A 13-->0.8A
		else {
...
int led_gpio_flash_probe(struct platform_device *pdev)
{
	...
	flash_led->gpio_flash_type_state_default = pinctrl_lookup_state(flash_led->pinctrl,
		"camera_flash_type_default");
	if (IS_ERR(flash_led->gpio_flash_type_state_default)) {
		pr_err("%s:can not get active pinstate\n", __func__);
		return -EINVAL;
	}
	rc = pinctrl_select_state(flash_led->pinctrl,
		flash_led->gpio_flash_type_state_default);
	if (rc)
		pr_err("%s:set state failed!\n", __func__);
	...
	flashType = of_get_named_gpio(node, "qcom,flash-type", 0);
	if (flashType < 0) {
		dev_err(&pdev->dev,
			"Looking up %s property in node %s failed. rc =  %d\n",
			"flash-now", node->full_name, flashType);
		goto error;
	} else {
		rc = gpio_request(flashType, "FLASH_TYPE");
		if (rc) {
			dev_err(&pdev->dev,
				"%s: Failed to request gpio %d,rc = %d\n",
				__func__, flashType, rc);
			goto error;
		}
	}

	rc = gpio_direction_input(flashType);
	
	flashTypevale = gpio_get_value_cansleep(flashType);
