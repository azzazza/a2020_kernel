...
static ssize_t msm_sensor_show_dev_info(struct device *dev, struct device_attribute *attr, char *buff)
{
	char *buff_bak = buff;
	...
	if (sensordata) {
		buff += sprintf(buff, "name = %s, position = %d", sensordata->sensor_name, sensordata->sensor_info->position);
	}

	return buff - buff_bak;
}

static DEVICE_ATTR(dev_info, S_IRUGO, msm_sensor_show_dev_info, NULL);
/* End add */
	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	*  sensor control structure */
	...
	device_create_file(&s_ctrl->msm_sd.sd.devnode->dev, &dev_attr_dev_info); // Add by ChenZiLi
	return rc;
