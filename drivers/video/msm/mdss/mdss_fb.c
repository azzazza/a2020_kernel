...
static const char *mdss_mdp_panel_type_tostring(u32 type)
{
	switch (type) {
	case NO_PANEL:
		return "none";
...
static ssize_t mdss_mdp_show_dev_info(struct device *dev, struct device_attribute *attr, char *buff)
{
	char *buff_bak = buff;
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	...
	if (panel_info) {
		buff += sprintf(buff, "name = %s, ", panel_info->panel_name);
	}
	buff += sprintf(buff, "type = %s, bus_width = %d, width = %d, height = %d\n",
		mdss_mdp_panel_type_tostring(panel_info->type), panel_info->bpp, panel_info->xres, panel_info->yres);
...
static DEVICE_ATTR(dev_info, S_IRUGO, mdss_mdp_show_dev_info, NULL); // Add by ChenZiLi
static struct attribute *mdss_fb_attrs[] = {
	...
	&dev_attr_dev_info.attr, // Add by ChenZiLi
	NULL,
};
