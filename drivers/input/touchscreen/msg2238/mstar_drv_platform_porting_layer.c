...
#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
struct input_dev *ps_inputdevice = NULL;
#endif
static int _gIrq = -1;
...
{
	int err;
	err = enable_irq_wake(_gIrq);
	if(err){
		pr_err("####%s() enable_irq_wake fail ###########\n",__func__);
	}		
}
...
{
	int err;
	err = disable_irq_wake(_gIrq);
	if(err){
		pr_err("####%s() disable_irq_wake fail ###########\n",__func__);
	}
}	
...
#ifdef CONFIG_ENABLE_REGULATOR_POWER_ON
void DrvPlatformLyrTouchDeviceRegulatorPowerOn(bool flag)
{
    int ret;
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
    DBG("*** %s() ***\n", __func__);
	...
	if (ret) {
		printk("*** %s() *** Regulator g_ReguVdd enable failed rc=%d\n",__func__, ret);
	}
        mdelay(20); //mdelay(100);

        ret = regulator_enable(g_ReguVcc_i2c);
	if (ret) {
		printk("*** %s() *** Regulator g_ReguVcc_i2c enable failed rc=%d\n",__func__, ret);
	}
        mdelay(20);
    } else {
        ret = regulator_disable(g_ReguVdd);
	if (ret) {
		printk("*** %s() *** Regulator g_ReguVdd disable failed rc=%d\n",__func__, ret);
	}		
        mdelay(20); //mdelay(100);

        ret = regulator_disable(g_ReguVcc_i2c);
	if (ret) {
		printk("*** %s() *** Regulator g_ReguVcc_i2c disable failed rc=%d\n",__func__, ret);
	}		
        mdelay(20);
    }
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
...
int msg22xx_ts_pinctrl_init(struct i2c_client *client)
{
	int retval;
	  u32  flags;
	...
	printk("##########reset_gpio = %d ,irq_gpio %d \n",reset_gpio,irq_gpio);
	if (irq_gpio < 0)
		return irq_gpio;
	...
	printk("*******************msg22xx_ts_pinctrl_init########\n");
	printk("********addr=%p%p%p%p#######\n",ts_pinctrl,pinctrl_state_active,pinctrl_state_suspend,pinctrl_state_release);
	return 0;
