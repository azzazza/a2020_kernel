	...
	struct device_attribute *attr, char *buff)
{
	...
	mutex_lock(&mdev->lock);
	wake_lock(&mdev->wake_lock);
	if (!mdev || !mdev->get_firmware_id || !mdev->client) {
		fw_mdev_err("mdev || !mdev->get_firmware_id || !mdev->client NULL\n");
		wake_unlock(&mdev->wake_lock);
		mutex_unlock(&mdev->lock);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, const char *buff, size_t size)
{
	...
	mdev->fw_size = simple_strtoul(buff, NULL, 10);
...
int fw_mdev_register(struct firmware_miscdevice *mdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(fw_mdev_device_attrs); i++) {
		ret = device_create_file(mdev->dev.this_device, &fw_mdev_device_attrs[i]);
		if (ret)
...
err_create_file:
	while(--i >= 0)
		device_remove_file(mdev->dev.this_device, &fw_mdev_device_attrs[i]);
